...
static sys_semaphore_t audioplayer_semaid;
static sys_semaphore_attribute_t audioplayer_sema_attr;
...
static void audio_add_callback(int voice)
{
    ...
    if (private_audio.pcm_indx >= READ_SAMPLES && (private_audio.flag & 2) == 2) {
        ...
        if (SND_AddVoice (VOICE, (void *) private_audio.pcmout[private_audio.pcmout_pos], private_audio.pcm_indx << 1) == 0) {
            ...
            sys_semaphore_post(audioplayer_semaid, 1);
        }
    } else {    
...
//        sys_ppu_thread_yield();
        ...
        if (private_audio.flag) {
            ...
            sys_semaphore_wait(audioplayer_semaid, 0);  // wait only when i have samples to send
    ...
    sys_semaphore_destroy(audioplayer_semaid);
...
void StopAudio()
{
    ...
    if (audio_thread_running > 0) {
        ...
        while (((volatile int) audio_thread_running) != 0) {
            usleep(5000); sys_semaphore_post(audioplayer_semaid, 1);
        }
    
        sys_ppu_thread_join(h_audioplayer, NULL);
    }
}
...
int DecodeAudio(void *audiofile, int size, short *samples, int *size_samples, int *freq, int *is_stereo)
{
    ...
    if ((err = my_ov_open((void *) private_audio.fd, &private_audio.vf, NULL, 0)) < 0) {
        ...
        if (err == OV_ENOTVORBIS) {
            ...
            || mpg123_getformat(private_audio.mh, &private_audio.rate, &private_audio.channels, &private_audio.encoding) != MPG123_OK) {
...
int PlayAudiofd(FILE * fd, int time_pos, int mode)
{
    ...
    memset(&audioplayer_sema_attr, 0, sizeof(sys_semaphore_attribute_t));

    audioplayer_sema_attr.attr_protocol = 0x2;
    audioplayer_sema_attr.attr_pshared  = 0x2 << 8;

    sys_semaphore_create(&audioplayer_semaid, &audioplayer_sema_attr, 0, 1);

    if (sys_ppu_thread_create(&h_audioplayer, (void *) audio_player_thread, (u64) & private_audio, PLAYER_PRIORITY, STACKSIZE,
